...
export const HeaderBar: FC<HeaderT> = forwardRef<HTMLButtonElement>((props, ref) => {
  const isDesktopOrLaptop = useMediaQuery({ minDeviceWidth: 1224 })
  const navigate = useNavigate()
  const [dropDownCars, { open: openDropDown, close: closeDropDown }] = useBoolean(false)
  ...
  return (
    <header className={styles.headerWrapper}>
      <nav className={`${styles.headerWrapper_header} ${('color' in props && props.color) || mobileMenu ? styles.scrolledBg : ''}`}>
        <div className={styles.headerWrapper_header_logo}>
          <a href={Path.Home} rel="nofollow">
            <img src={'https://www.shineray.com/upload/logo/1703907286103471.png'} />
          </a>
        </div>
        {isDesktopOrLaptop ? (
          <div className={styles.headerWrapper_header_buttonsBlock}>
            <Button text="Модельный ряд" ref={ref} data-popup="menu-cars" onClick={() => navigate(Path.Cars)} />
            <Button text="Дилеры" />
          ...
          <div className={styles.headerWrapper_header_buttonsBlock}>
            <IconButton size="large" edge="start" aria-label="open-mobile-menu" sx={{ mr: 2, color: 'white' }} onClick={toggleMobileMenu}>
